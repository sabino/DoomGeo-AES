...
| Area | Status |
| --- | --- |
| WAD conversion | Converts E1M1 map lumps, player start, doors, exits, secrets, damaging sectors, texture classes, sector heights, and runtime things into a centered `96x72` Neo Geo grid. The default conversion now simplifies tiny solid-line noise at that scale instead of preserving every WAD line as a full-height raycast obstacle. Generated map arrays are split into `doom_map_generated.h` declarations and `doom_map_generated.c` table definitions. |
