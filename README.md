...
| RIPDOOM-lite geometry | `make ripdoom-map ripdoom-check` emits and validates a compact, SNES-inspired geometry package from real Doom `VERTEXES`, `LINEDEFS`, `SIDEDEFS`, `SECTORS`, `SEGS`, `SSECTORS`, `NODES`, `REJECT`, `BLOCKMAP`, and `THINGS`. This is the next converter contract for BSP/seg-driven rendering; it is not consumed by the runtime yet. |
