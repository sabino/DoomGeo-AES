...
def sprite_scale_tiles(iwad, zip_member, sprite_name, scales):
    ...
    return tiles, meta, palette


def write_palette_header(path, wall_palette, wall_source, hud_palette, hud_source, ceiling_palette, ceiling_source, floor_palette, floor_source, weapon_palette, weapon_source, sprite_meta, sprite_palette, sprite_source):
    ...
    with open(path, "w", encoding="ascii") as f:
        ...
        f.write(f"#define ENEMY_PATCH_SOURCE \"{sprite_source}\"\n")
        f.write("static const u8 g_enemy_palette_rgb[ENEMY_PALETTE_COLORS][3] = {\n")
        for rgb in sprite_palette:
            r, g, b = to_neo_rgb(rgb)
            f.write(f"    {{{r},{g},{b}}},\n")
        f.write("};\n\n")
...
def main():
    ...
    sprite_tiles, sprite_meta, sprite_palette = sprite_scale_tiles(args.iwad, args.zip_member, args.sprite_frame, scales)
