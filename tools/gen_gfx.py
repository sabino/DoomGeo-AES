...
DOOR_ATLAS_BASE = WALL_ALT_ATLAS_BASE + WALL_ATLAS_TILES
FLAT_COLS = 4
FLAT_ROWS = 4
...
def write_palette_header(path, wall_palette, wall_source, wall_alt_palette, wall_alt_source, door_palette, door_source, hud_palette, hud_source, ceiling_palette, ceiling_source, floor_palette, floor_source, weapon_palette, weapon_source, sprite_defs, sprite_meta, sprite_palettes):
    ...
    with open(path, "w", encoding="ascii") as f:
        ...
        f.write(f"#define WALL_ALT_TEXTURE_SOURCE \"{wall_alt_source}\"\n")
        f.write("static const u8 g_wall_alt_palette_rgb[WALL_ALT_PALETTE_COLORS][3] = {\n")
        for rgb in wall_alt_palette:
            r, g, b = to_neo_rgb(rgb)
            f.write(f"    {{{r},{g},{b}}},\n")
        f.write("};\n\n")
...
def main():
    ...
    ap.add_argument("--wall-alt-texture", default=WALL_ALT_TEXTURE, help="Second Doom wall texture atlas for per-cell map texture classes")
    ...
    wall_alt_tiles, wall_alt_source, wall_alt_palette = wall_texture_tiles(args.iwad, args.zip_member, args.wall_alt_texture)
    ...
    tiles = [tile_blank(), wall_tiles[0], tile_solid()] + wall_tiles[1:] + wall_alt_tiles[1:] + door_tiles[1:] + ceiling_tiles + floor_tiles + hud_tiles + face_tiles + weapon_cache + sprite_tiles
    ...
    print(f"  alt wall texture: {wall_alt_source} atlas={WALL_ALT_ATLAS_BASE}..{WALL_ALT_ATLAS_BASE + WALL_ATLAS_TILES - 1} ({WALL_ATLAS_COLS}x{WALL_ATLAS_ROWS})")
